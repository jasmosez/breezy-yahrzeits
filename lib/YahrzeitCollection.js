...
export class YahrzeitCollection {
    ...
    async init () {
        ...
        try {
            ...
            console.error(this._yahrzeitFetchError, error)
            process.exit()
        }
    }
        ...
        this.processedData = this.data.map(form => {
            try {
                ...
                if (dateOfPassing > now) {
                    throw new RangeError(`Date of passing is in the future: ${dateOfPassing.toLocaleDateString('en-US', localeOptions)}. Today is ${new Date().toLocaleDateString('en-US', localeOptions)}.`)
                }
...
`)
    ...
    applyMonthFilter(targetMonth, targetYear) {
        if (targetMonth === 0) {
            this.filteredProcessedData = this.processedData
        } else {
            this.filteredProcessedData = this.processedData.filter(form => {
                return form.month_number === parseInt(targetMonth) && form.year === targetYear
            })
        }
    }
    ...
    async addProfileStatusData() {
        ...
        this.formsWithProfileIds = await Promise.all(filteredProcessedDataWithProfileIds.map(async form => {
            ...
            try {
                ...
                let profile_email
                const email = profile.details[profileEmailListField]?.find(el => el[isPrimaryField] === '1' && el[allowBulkField] === '1')
                if (!email?.address) {
                ...
                form.member_status = profile.details[profileStatusField]?.name || "Unknown"
        ...
        try {
            ...
            console.error(`Error filtering forms by member status: ${error}`)
        }
    }
    ...
    static async createYahrzeitCollection({targetMonth, targetYear}) {
        let collection = new YahrzeitCollection();
        await collection.init();
        collection.addCalculatedData();
        collection.applyMonthFilter(targetMonth, targetYear);
