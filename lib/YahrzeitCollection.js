        ...
        this.processedData = this.data.map(form => {
            try {
                ...
                if (dateOfPassing > now) {
                    throw new RangeError(`Date of passing is in the future: ${dateOfPassing.toLocaleDateString('en-US', localeOptions)}. Today is ${new Date().toLocaleDateString('en-US', localeOptions)}.  Form: ${form}`)
                }
