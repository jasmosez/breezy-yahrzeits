...
import {OBSERVATION_SELECTION, GREGORIAN_CAL, HEBREW_CAL, GREGORIAN_DATE_OF_PASSING, SUNSET_SELECTION, AFTER_SUNSET, EMAIL, ENGLISH_NAME_DECEASED, FIRST_NAME_MOURNER, PROFILE, RELATIONSHIP, PROFILE_STATUS, PROFILE_EMAIL_LIST, BEFORE_SUNSET} from './form_constants.js'
...
const getYahr = (hdDateOfPassing) => {
    ...
    if (!thisYearYahr) {
        if (hdDateOfPassing.getFullYear() === today.getFullYear()) {
...
const addProfileStatus = async (filteredforms) => {
    ...
    const updatedForms = await Promise.all(formsWithProflies.map(async form => {
        try {
            ...
            const updatedForm = {
                ...form,
                profile_first_name: respJson.first_name,
                profile_last_name: respJson.last_name,
                member_status: respJson.details[PROFILE_STATUS].name
            }

            const email = respJson.details[PROFILE_EMAIL_LIST].find(el => el.is_primary === '1' && el.allow_bulk === '1')
            if (!email?.address) {
                console.error(`Missing profile email. id: ${respJson.id}, last name: ${respJson.last_name}`)
            } else {
                updatedForm.profile_email = email.address
            }

            return updatedForm
        } catch(err){
            console.log(`Failed to get profile info for id ${form.person_id}`, err)
        }
    }))
    return updatedForms
}
...
const mapResponses = (json) => {
    let errorCount = 0
    const mappedJson = json.map(form => {
        try {
            ...
            form.response.year = yahr.getFullYear()
    ...
    console.log('-----------------------------------------------')
    console.log(`${errorCount} forms skipped due to errors`)
    console.log('-----------------------------------------------')
    console.log('')
    return mappedJson
}
...
const mapFilterMap = async (monthFilter, json) => {
    ...
    if (!!monthFilter) {
        mappedForms = mappedForms.filter(form => {
            return form.response.month_number === monthFilter 
                    && form.response.year === determineTargetYear(monthFilter)
        })
