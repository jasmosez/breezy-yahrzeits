import fs from 'fs'
import { ENGLISH_NAME_DECEASED, FIRST_NAME_MOURNER, LAST_NAME_MOURNER, PROFILE, RELATIONSHIP, MEMBER, DECEASED } from "./form_constants.js";
...
const saveToText = (monthFilter, filteredForms) => {
    const membersAndDeceased = filteredForms.filter(form => form.member_status === MEMBER || form.member_status === DECEASED)
    const textDataObj = makeTextData(monthFilter, membersAndDeceased)
    if (!textDataObj.success) {
        throw 'Could not process yahrzeit text for shabbat email'
    }

    fs.writeFileSync(savedTXT, textDataObj.outputString)
